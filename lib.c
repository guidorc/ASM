...
void listRemove(list_t* l, void* data){
    ...
    listElem_t* nodoAux = l->first;
    if (l->size != 0) {
        while(actual->next!=NULL){
          nodoAux=actual;
          int32_t cmpRes = fc(actual->data, data);
          if(cmpRes==0){
            if (actual->prev == NULL) { // Es el primer nodo de la lista
                l->first = actual->next;
                ...
                free(nodoAux);
                l->size = l->size - 1;
            }else if (actual->next == NULL) { // Es el último nodo de la lista
                l->last = actual->prev;
...
list_t* treeGet(tree_t* tree, void* key) {
    ...
    funcCmp_t* fc = getCompareFunction(tree->typeKey);
	  ...
	  while( actual != NULL) {
        int32_t cmpRes = fc(actual->key, key);
        if (cmpRes == 0) {
...
void treeRemove(tree_t* tree, void* key, void* data) {
    ...
    treeNode_t* actual = tree->first;
    while (actual != NULL) {
        int32_t cmpRes = fc(key, actual->key);
        if(cmpRes == 0){   // el nodo actual tiene el dato
